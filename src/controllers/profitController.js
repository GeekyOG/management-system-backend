...
// Helper function to calculate total profit based on time range and businessId
const calculateProfit = async (startDate, endDate, businessId) => {
  const sales = await SaleItem.findAll({
    ...
    include: [
      { model: Sale, where: { status: "completed", businessId } }, // Include businessId filter
      { model: Product },
    ],
  });
...
exports.getProfit = async (req, res) => {
  ...
  const { businessId } = req; // Ensure businessId is extracted from the request context
  ...
  try {
    ...
    // Calculate profit within the date range and for the specific business
    const totalProfit = await calculateProfit(start, end, businessId);
